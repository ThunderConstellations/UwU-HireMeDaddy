...
async function waitForJobDetails(timeout = 15000) {
  ...
  while (Date.now() - start < timeout) {
    const title = document.querySelector('.e11nt52q1, .h1');
    const company = document.querySelector('.e11nt52q2, .h2');
...
export async function run() {
  if (!(await waitForJobDetails())) {
    ...
    await logError('Job details not found in time', { site: 'Glassdoor', url: window.location.href });
    return;
  }
  let lastError = null;
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      let location = document.querySelector('.e11nt52q3, .location, .job-location, .job-header-location')?.innerText || '';
      if (!location) {
  ...
  const { logError } = await import('../../utils/logger.js');
  await logError('Auto-apply failed after 3 attempts', { site: 'Glassdoor', url: window.location.href, error: lastError });
}